...
{
    RgbColor color;
    for (uint16_t indexPixel = 0; indexPixel < strip.PixelCount()/4; indexPixel++)
    {
        color = strip.GetPixelColor(indexPixel);
        color.Darken(darkenBy);
        strip.SetPixelColor(indexPixel, color);
        strip.SetPixelColor(59-indexPixel, color);
        strip.SetPixelColor(indexPixel+60, color);
        strip.SetPixelColor(119-indexPixel, color);
    }
}
